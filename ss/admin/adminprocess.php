...
class AdminProcess
{
    ...
    * Phil: changed for v0.2 db; does not delete an active user
    */
...
/*         $q = "DELETE FROM ".TBL_USERS." WHERE username = '$subuser'";*/
      	 $q = "SELECT user_id, active_id ".
      	 	  "FROM known_user LEFT JOIN active_user ".
      	 	  "ON user_id = active_id ".
      	 	  "WHERE username = '$subuser'";
    	 
         $result = $database->query($q) or die();
         $dbarray = mysql_fetch_array($result);
	     $active_id = $dbarray['active_id'];
	     $user_id = $dbarray['user_id'];
	     if ($active_id == $user_id)
	     {
	     	/* don't delete an active user */
	     	/* TODO: create an admin function to log users out */
	     	$field = "deluser";
	     	$form->setError($field, "* user is active");
	     	$_SESSION['value_array'] = $_POST;
    	    $_SESSION['error_array'] = $form->getErrorArray(); 	
	     }
	     else 
	     {
	     	/* user is not active so go ahead and delete */
	        $q = "DELETE FROM ".TBL_KNOWN_USER." WHERE user_id = $user_id";
	        $database->query($q) or die(mysql_error());
	        $q = "DELETE FROM ".TBL_USERS." WHERE id = $user_id";
	        $database->query($q) or die(mysql_error());
	        $database->calcNumActiveUsers();
	     }
         header("Location: ".$session->referrer);
      }
   }
   ...
   function procBanUser(){
      ...
      /* Ban user from member system */
         ...
         $q = "SELECT user_id FROM ".TBL_KNOWN_USER." WHERE username = '$subuser'";
         $result = $database->query($q);
         $dbarray = mysql_fetch_array($result);
         $id = $dbarray["user_id"];

         $q = "INSERT INTO ".TBL_BANNED_USERS." VALUES ($id, $session->time)";
         $database->query($q);
         header("Location: ".$session->referrer);
      }
   }
    ...
    * enables that username again.
    * Phil: No changes for v0.2 db.
    */
   function procDeleteBannedUser(){
      ...
      /* Delete user from database */
         ...
         $q = "DELETE FROM ".TBL_BANNED_USERS." WHERE banned_id = $id";
         $database->query($q);
         header("Location: ".$session->referrer);
      }
   }
   ...
   function checkUsername($uname, $ban=false){
      ...
      if(!$subuser || strlen($subuser = trim($subuser)) == 0){
         ...
         if(/* Phil: Not enforcing this rule: strlen($subuser) < 5 || */
         	strlen($subuser) > 30 ||
