 ...
 *
 * Original code version written by: Jpmaster77 a.k.a. The Grandmaster of C++ (GMC)
 * Updates by Pred MacLinty
 * @version 0.3 dev
 */
...
/*      $q = "SELECT * FROM ".TBL_USERS." WHERE username = '$username'";*/
   ...
   /**
    * trackUser - populates the tracking table with information about the user's calling
    *             environemnt and site activity
    * @author Pred
    * @since 0.3 dev
    * @version 0.3 dev
    * @param $id - integer PK of the user table entry being tracked.
    * @param $time - integer timestamp of the activity being tracked, eg. UNIX_TIMESTAMP() from MySQL
    * @param $activity - string description, either a script name or a process word
    * @return void
    */

   function trackUser ($id, $time, $activity)
   {
   	  if (!TRACK_VISITORS) return;

   	  if (!empty($_SERVER['HTTP_USER_AGENT']))  $querydata['agent'] = $_SERVER['HTTP_USER_AGENT'];
   	  if (!empty($_SERVER['HTTP_REFERER']))  $querydata['referrer'] = $_SERVER['HTTP_REFERER'];
   	  ...
   	  if (!empty($ip))
   	  {
   	  	$querydata['ipaddress'] = $ip;
   	  	$querydata['ipnumber'] = sprintf("%u", ip2long(long2ip(ip2long($ip))));
   	    if ($ip != '127.0.0.1')
   	    {
   	  ...
   	  $q = "INSERT INTO ".TBL_TRACKING." (".$columns.") VALUES (".$values.")";
      mysql_query($q, $this->connection) or die(mysql_error());
	  $q = "INSERT INTO ".TBL_USER_TRACKING." (user_id, tracking_id) VALUES ($id, LAST_INSERT_ID())";
	  mysql_query($q, $this->connection) or die(mysql_error());
   }
    ...
    * Phil: changed for v0.2 db
    * Phil: update for tracking: insert activity into tracking table
    */
...
/*      $q = "UPDATE ".TBL_USERS." SET timestamp = '$time' WHERE username = '$username'";*/
   ...
   function addActiveGuest($ip, $time)
   {
      ...
      if ($rowsreturned <= 0)
      {
	      ...
	      mysql_query($q, $this->connection) or die(mysql_error());
	      $q = "SELECT LAST_INSERT_ID() AS guest_id";
	      $result = mysql_query($q, $this->connection) or die(mysql_error());
      ...
      $this->trackUser($id, $time, $_SERVER['SCRIPT_NAME']);
      $this->calcNumActiveGuests();
   }
